		...
		<div id="wrappermiddle">
			...
			<?php
				...
				if(isset($_POST['Amount'])){
					if ($_POST['Pay']){
						$Amount=$_POST["Amount"];
						$sql="SELECT Balance,Debit FROM company
						...
						if($debit>0){
							if($balance>=$Amount){
								...
								if($new_debit<0){
									...
									$debit=0;
									$sql="UPDATE company SET company.Debit ='$debit',company.Balance ='$balance'WHERE company.Company_num =".$ClientID;
									if(mysqli_query($conn,$sql)){
										echo "Payment successful<br>";
									}else{ echo "Error <br>";}
								}else{
									$sql="UPDATE company SET company.Debit ='$new_debit',company.Balance ='$balance'WHERE company.Company_num =".$ClientID;
									if(mysqli_query($conn,$sql)){
										echo "Payment successful<br>";
									}else{ echo "Error <br>";}
								}
							}else{
								echo "Company has not enough balance to pay the amount<br>";
							}
						}else{
							echo "Company has no debit to CCC<br>";
						}
					}
				}
			?>

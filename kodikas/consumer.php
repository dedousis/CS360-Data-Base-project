...
<div id="wrapper">
	<div id="wrappertop"></div>
		<div id="wrappermiddle">
			...
			$sql="SELECT Balance,Debit FROM consumer WHERE consumer.Consumer_num=".$ID;
			$result=mysqli_query($conn, $sql);
			$row=mysqli_fetch_assoc($result);
			$_SESSION["Balance"]=$row["Balance"];
			$_SESSION["Debit"]=$row["Debit"];
			echo "ID: ".$ID."<br>  Balance: ".$_SESSION["Balance"]."<br> Debit: ".$_SESSION["Debit"]."<br><br>";
			?>
			<form action="Buy_consumer.php" method="POST">
				<input id="button" type="submit" name="Buy" value="Buy" style="width: 300px; height: 40px;" />
			</form>

			<form action="Return_consumer.php" method="POST">
			<input id="button" type="submit" name="Return" value="Return" style="width: 300px; height: 40px;"/>
			</form>

			<form action="Payment_consumer.php" method="POST">
			<input id="button" type="submit" name="Payment" value="Payment" style="width: 300px; height: 40px;"/>
			</form>

			<form action="Transcations_consumer.php" method="POST">
			<input id="button" type="submit" name="Transcations" value="Transcations" style="width: 300px; height: 40px;"/>
			</form>
			...
			<?php
				if(isset($_POST['deletebutton'])){
					if ($_POST['deletebutton']){
						...
						$debit="SELECT Debit FROM consumer WHERE consumer.Consumer_num=".$ID;
						$result = mysqli_query($conn, $debit);
						$row=mysqli_fetch_assoc($result);
						
						if($row["Debit"]==0)
						{
							$sql="DELETE FROM consumer WHERE consumer.Consumer_num=".$ID;
							$result = mysqli_query($conn, $sql);
							if($result)
							{

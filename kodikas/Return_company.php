		...
		<div id="wrappermiddle">
			...
			<center><b><?php
				...
				$sql= "SELECT * FROM transaction
						WHERE transaction.Amount<>-1 AND transaction.Returned=0 AND transaction.Company_num=".$ClientID;
			...
			<?php
				if(isset($_POST['Trans_id'])){
					if ($_POST['Continue']){
						$id=$_POST["Trans_id"];
						$sql= "SELECT * FROM transaction
							WHERE transaction.Trans_id='$id' AND transaction.Company_name LIKE '%{$ClientName}%'";
									...
									if(mysqli_query($conn,$sql))
									{
										...
										if(mysqli_query($conn,$sql))
										{
											$sql="INSERT INTO transaction(Trans_id,Client_name,Merchant_name,Amount,Date,Type,Merchant_num,Consumer_num,Company_num,Employee_ID,Company_name,Returned)
												VALUES('$trans_id',NULL,'$Merchant_name',-1,'$date','$Trans_type','$Merchant_num',NULL,'$ClientID','$EmployeeID','$ClientName','0')";
											if(mysqli_query($conn,$sql)){	
					...
					echo("<script>location.href = 'http://127.0.0.1/project/Return_company.php';</script>");
					}
				}

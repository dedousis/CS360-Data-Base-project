...
<body>
	...
	$MerchanttID=$_SESSION["ID"];
	$type=$_SESSION["Type"];
	echo "<h2>".$MerchantName."</h2>"."<br><br>id: ".$MerchanttID."<br><br> ";
...
<?php echo "<br>"?>
End Date:  <input type="date" name="End_Date" data-date-inline-picker="true" /><br>	
...
if(isset($_POST['Start_Date'])and isset($_POST['End_Date'])){
		if ($_POST['Proceed'])
		{
			...
			$sql="SELECT * FROM transaction WHERE transaction.Date BETWEEN '$Start_date' AND '$End_date'AND transaction.Merchant_num=".$MerchanttID;
			if($result=mysqli_query($conn, $sql)){
				if(mysqli_num_rows($result)!=0){
					...
					<tr> <td><font color=black>Transaction id</font></td> <td><font color=black>Consumer Name</font></td><td><font color=black>Merchant Name</font></td>
					...
					<td><font color=black>Consumer Num </font></td></tr>";
					while($row=mysqli_fetch_assoc($result)){
						echo "<tr> <td>".$row["Trans_id"]."</td> <td>".$row["Client_name"]."</td><td>".$row["Merchant_name"]."</td><td>".$row["Amount"].
						"</td><td>".$row["Date"]."</td><td>".$row["Type"]."</td><td>".$row["Merchant_num"]."</td><td>".$row["Consumer_num"]."</font></td></tr>";  
					}

								...
								if($flag==0){
									...
									if($Tran_type == "Debit"){						//in case the transaction type is Debit
										...
										if($Amount>$row["Balance"])
										{
											...
											$url="http://127.0.0.1/project/Buy_consumer.php";
										}else{
											$date=date("Y/m/d");
											...
											VALUES('$trans_id','$ClientName','$MerchantName','$Amount','$date','$Tran_type','$MerchantID','$ClientID',NULL,NULL,NULL,'0')";
										...
										$sql="SELECT Debit,Credit_Limit FROM consumer
												WHERE consumer.Consumer_num=".$ClientID;
										$result = mysqli_query($conn,$sql);
										$row=mysqli_fetch_assoc($result);
										if($row["Credit_Limit"]>=$Amount){
											...
											if(mysqli_query($conn,$sql))
											{
												...
												$sql="UPDATE consumer SET consumer.Debit ='$neo_debit'WHERE consumer.Consumer_num =".$ClientID;
												if(mysqli_query($conn,$sql)){

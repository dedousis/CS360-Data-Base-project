		...
		<div id="wrappermiddle">
			...
			<?php
				...
				if(isset($_POST['Amount'])){
					if ($_POST['Pay']){
						$Amount=$_POST["Amount"];
						$sql="SELECT Total_Profit,Debit FROM merchant
										WHERE merchant.Merchant_num=".$ClientID;
						...
						if($debit>0){
							if($profit>=$Amount){
								...
								if($new_debit<0){
									$profit=$profit + $debit;
									$debit=0;
									$sql="UPDATE merchant SET merchant.Debit ='$debit',merchant.Total_Profit ='$profit'WHERE merchant.Merchant_num =".$ClientID;
									if(mysqli_query($conn,$sql)){
										echo "Payment successful<br>";
									}else{ echo "Error <br>";}
								}else{
									$sql="UPDATE merchant SET merchant.Debit ='$new_debit',merchant.Total_Profit ='$profit'WHERE merchant.Merchant_num =".$ClientID;
									if(mysqli_query($conn,$sql)){
										echo "Payment successful<br>";
									}else{ echo "Error <br>";}
								}
							}else{
								echo "Merchant has not enough balance to pay the amount<br>";
							}
						}else{
							echo "Merchant has no debit to CCC<br>";
						}
					}
				}
			?>
